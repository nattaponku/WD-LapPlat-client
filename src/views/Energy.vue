            ...
            <div class="row">
              <div class="col-sm">
                <!-- <input type="checkbox" 
                  ...
                  v-model="signal.Vpv1" 
                  @change="initBigChart()"> 
                  <label for="signal.Vpv1"> Vpv1 </label> -->
                <base-checkbox class="mb-3" v-model="signal.Vpv1" @input="initBigChart()">
                    Vpv1
                </base-checkbox>
              </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Ipv1" @input="initBigChart()" >
                    Ipv1
                </base-checkbox>
              </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Ppv1" @input="initBigChart()">
                    Ppv1
                </base-checkbox>
              </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Vpv2" @input="initBigChart()">
                    Vpv2
                </base-checkbox>
              </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Ppv2" @input="initBigChart()">
                    Ppv2
                </base-checkbox>
              </div>
              <div class="w-100"></div> <!-- new row -->
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Ipv2" @input="initBigChart()">
                    Ipv2
                </base-checkbox>
              </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Vac" @input="initBigChart()">
                    Vac
                </base-checkbox>
                </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Iac" @input="initBigChart()">
                    Iac
                </base-checkbox>
                </div>
              <div class="col-sm">
                <base-checkbox class="mb-3" v-model="signal.Pac" @input="initBigChart()">
                    Pac
                </base-checkbox>
              </div>
              <div class="col-sm">
...
function filterDataset(srcDataset,filterObj) {
  ...
  for (var i=0;i<srcDataset.length;i++){
    switch(srcDataset[i].label){
      case 'Vpv1' : 
        if (filterObj.Vpv1){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Ipv1' : 
        if (filterObj.Ipv1){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Ppv1' : 
        if (filterObj.Ppv1){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Vpv2' : 
        if (filterObj.Vpv2){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Ipv2' : 
        if (filterObj.Ipv2){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Ppv2' : 
        if (filterObj.Ppv2){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Vac' : 
        if (filterObj.Vac){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Iac' : 
        if (filterObj.Iac){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
      case 'Pac' : 
        if (filterObj.Pac){selectedBigLineChartDatasets.push(srcDataset[i]);}
        break;
    }
  }
...
export default {
  ...
  methods: {
    ...
    initBigChart() {
      ...
      bigLineChartData.datasets = filterDataset(bigLineChartData.datasets,this.signal);
